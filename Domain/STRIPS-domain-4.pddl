(define (domain trucks-project-4) 
    ...
    (:action turn_left_from_east_zero_bridge
        :parameters (?p - agent ?from ?to - square ?toDir - direction)
        :precondition (and (not (on-ground ?p)) (upper-floor ?from) (upper-floor ?to) (adj ?from ?to ?toDir) (is-north ?toDir) (facing-east ?p) (at ?p ?from) (zero-turn ?p) (not (jammed ?p)))
    ...
    (:action turn_left_from_east_one_bridge
        :parameters (?p - agent ?from ?to - square ?toDir - direction)
        :precondition (and (not (on-ground ?p)) (upper-floor ?from) (upper-floor ?to) (adj ?from ?to ?toDir) (is-north ?toDir) (facing-east ?p) (at ?p ?from)  (first-turn ?p) (not (jammed ?p)))
    ...
    (:action turn_left_from_south_zero_bridge
        :parameters (?p - agent ?from ?to - square ?toDir - direction)
        :precondition (and (not (on-ground ?p)) (upper-floor ?from) (upper-floor ?to) (adj ?from ?to ?toDir) (is-east ?toDir) (facing-south ?p) (at ?p ?from) (zero-turn ?p) (not (jammed ?p)))
    ...
    (:action turn_left_from_south_one_bridge
        :parameters (?p - agent ?from ?to - square ?toDir - direction)
        :precondition (and (not (on-ground ?p)) (upper-floor ?from) (upper-floor ?to) (adj ?from ?to ?toDir) (is-east ?toDir) (facing-south ?p) (at ?p ?from) (first-turn ?p) (not (jammed ?p)))
    ...
    (:action turn_left_from_north_zero_bridge
        :parameters (?p - agent ?from ?to - square ?toDir - direction)
        :precondition (and (not (on-ground ?p)) (upper-floor ?from) (upper-floor ?to) (adj ?from ?to ?toDir) (is-west ?toDir) (facing-north ?p) (at ?p ?from) (zero-turn ?p) (not (jammed ?p)))
    ...
    (:action turn_left_from_north_one_bridge
        :parameters (?p - agent ?from ?to - square ?toDir - direction)
        :precondition (and (not (on-ground ?p)) (upper-floor ?from) (upper-floor ?to) (adj ?from ?to ?toDir) (is-west ?toDir) (facing-north ?p) (at ?p ?from) (first-turn ?p) (not (jammed ?p)))
    ...
    (:action turn_left_from_west_zero_bridge
        :parameters (?p - agent ?from ?to - square ?toDir - direction)
        :precondition (and (not (on-ground ?p)) (upper-floor ?from) (upper-floor ?to) (adj ?from ?to ?toDir) (is-south ?toDir) (facing-west ?p) (at ?p ?from) (zero-turn ?p) (not (jammed ?p)))
    ...
    (:action turn_left_from_west_one_bridge
        :parameters (?p - agent ?from ?to - square ?toDir - direction)
        :precondition (and (not (on-ground ?p)) (upper-floor ?from) (upper-floor ?to) (adj ?from ?to ?toDir) (is-south ?toDir) (facing-west ?p) (at ?p ?from) (first-turn ?p) (not (jammed ?p)))
    ...
    (:action turn_right_from_east_zero_bridge
        :parameters (?p - agent ?from ?to - square ?toDir - direction)
        :precondition (and (not (on-ground ?p)) (upper-floor ?from) (upper-floor ?to) (adj ?from ?to ?toDir) (is-south ?toDir)  (facing-east ?p) (at ?p ?from) (zero-turn ?p) (not (jammed ?p))) 
    ...
    (:action turn_right_from_east_one_bridge
        :parameters (?p - agent ?from ?to - square ?toDir - direction)
        :precondition (and (not (on-ground ?p)) (upper-floor ?from) (upper-floor ?to) (adj ?from ?to ?toDir) (is-south ?toDir)  (facing-east ?p) (at ?p ?from)  (first-turn ?p) (not (jammed ?p))) 
    ...
    (:action turn_right_from_south_zero_bridge
        :parameters (?p - agent ?from ?to - square ?toDir - direction)
        :precondition (and (not (on-ground ?p)) (upper-floor ?from) (upper-floor ?to) (adj ?from ?to ?toDir) (is-west ?toDir) (facing-south ?p) (at ?p ?from)  (zero-turn ?p) (not (jammed ?p))) 
    ...
    (:action turn_right_from_south_one_bridge
        :parameters (?p - agent ?from ?to - square ?toDir - direction)
        :precondition (and (not (on-ground ?p)) (upper-floor ?from) (upper-floor ?to) (adj ?from ?to ?toDir) (is-west ?toDir) (facing-south ?p) (at ?p ?from) (first-turn ?p) (not (jammed ?p)))
    ...
    (:action turn_right_from_north_zero_bridge
        :parameters (?p - agent ?from ?to - square ?toDir - direction)
        :precondition (and (not (on-ground ?p)) (upper-floor ?from) (upper-floor ?to) (adj ?from ?to ?toDir) (is-east ?toDir)  (facing-north ?p) (at ?p ?from)  (zero-turn ?p) (not (jammed ?p)))
    ...
    (:action turn_right_from_north_one_bridge
        :parameters (?p - agent ?from ?to - square ?toDir - direction)
        :precondition (and (not (on-ground ?p)) (upper-floor ?from) (upper-floor ?to) (adj ?from ?to ?toDir) (is-east ?toDir)  (facing-north ?p) (at ?p ?from) (first-turn ?p) (not (jammed ?p)))
    ...
    (:action turn_right_from_west_zero_bridge
        :parameters (?p - agent ?from ?to - square ?toDir - direction)
        :precondition (and (not (on-ground ?p)) (upper-floor ?from) (upper-floor ?to) (adj ?from ?to ?toDir) (is-north ?toDir) (facing-west ?p) (at ?p ?from) (zero-turn ?p) (not (jammed ?p)))
    ...
    (:action turn_right_from_west_one_bridge
        :parameters (?p - agent ?from ?to - square ?toDir - direction)
        :precondition (and (not (on-ground ?p)) (upper-floor ?from) (upper-floor ?to) (adj ?from ?to ?toDir) (is-north ?toDir) (facing-west ?p) (at ?p ?from) (first-turn ?p) (not (jammed ?p)))
    ...
    (:action move_forward_from_east_zero_bridge
        :parameters (?p - agent ?from ?to - square ?toDir - direction)
        :precondition (and (not (on-ground ?p)) (upper-floor ?from) (upper-floor ?to) (adj ?from ?to ?toDir) (is-east ?toDir) (facing-east ?p) (at ?p ?from) (zero-turn ?p) (not (jammed ?p)))
        :effect (and (not (at ?p ?from)) (at ?p ?to) (increase (total-cost) 1))
    )
    
    (:action move_forward_from_east_one_bridge
        :parameters (?p - agent ?from ?to - square ?toDir - direction)
        :precondition (and (not (on-ground ?p)) (upper-floor ?from) (upper-floor ?to) (adj ?from ?to ?toDir) (is-east ?toDir) (facing-east ?p) (at ?p ?from) (first-turn ?p) (not (jammed ?p)))
    ...
    (:action move_forward_from_south_zero_bridge
        :parameters (?p - agent ?from ?to - square ?toDir - direction)
        :precondition (and (not (on-ground ?p)) (upper-floor ?from) (upper-floor ?to) (adj ?from ?to ?toDir) (is-south ?toDir) (facing-south ?p) (at ?p ?from) (zero-turn ?p) (not (jammed ?p)))
        :effect (and (not (at ?p ?from)) (at ?p ?to) (increase (total-cost) 1)) 
    )
    
    (:action move_forward_from_south_one_bridge
        :parameters (?p - agent ?from ?to - square ?toDir - direction)
        :precondition (and (not (on-ground ?p)) (upper-floor ?from) (upper-floor ?to) (adj ?from ?to ?toDir) (is-south ?toDir) (facing-south ?p) (at ?p ?from) (first-turn ?p) (not (jammed ?p)))
    ...
    (:action move_forward_from_north_zero_bridge
        :parameters (?p - agent ?from ?to - square ?toDir - direction)
        :precondition (and (not (on-ground ?p)) (upper-floor ?from) (upper-floor ?to) (adj ?from ?to ?toDir) (is-north ?toDir) (facing-north ?p) (at ?p ?from) (zero-turn ?p) (not (jammed ?p)))
        :effect (and (not (at ?p ?from)) (at ?p ?to) (increase (total-cost) 1))
    )
    
    (:action move_forward_from_north_one_bridge
        :parameters (?p - agent ?from ?to - square ?toDir - direction)
        :precondition (and  (not (on-ground ?p)) (upper-floor ?from) (upper-floor ?to) (adj ?from ?to ?toDir) (is-north ?toDir) (facing-north ?p) (at ?p ?from) (first-turn ?p) (not (jammed ?p)))
        :effect (and (not (at ?p ?from)) (at ?p ?to) (zero-turn ?p) (not (first-turn ?p)) (increase (total-cost) 1))
    )
    ...
    (:action move_forward_from_west_zero_bridge
        :parameters (?p - agent ?from ?to - square ?toDir - direction)
        :precondition (and (not (on-ground ?p)) (upper-floor ?from) (upper-floor ?to) (adj ?from ?to ?toDir) (is-west ?toDir) (facing-west ?p) (at ?p ?from) (zero-turn ?p) (not (jammed ?p)))
        :effect (and (not (at ?p ?from)) (at ?p ?to) (increase (total-cost) 1))
    )
    
    (:action move_forward_from_west_one_bridge
        :parameters (?p - agent ?from ?to - square ?toDir - direction)
        :precondition (and (not (on-ground ?p)) (upper-floor ?from) (upper-floor ?to) (adj ?from ?to ?toDir) (is-west ?toDir) (facing-west ?p) (at ?p ?from) (first-turn ?p) (not (jammed ?p)))
        :effect (and (not (at ?p ?from)) (at ?p ?to) (zero-turn ?p) (not (first-turn ?p)) (increase (total-cost) 1))
    )
    ...
    (:action move_back_from_east_zero_bridge
        :parameters (?p - agent ?from ?to - square ?toDir - direction)
        :precondition (and (not (on-ground ?p)) (upper-floor ?from) (upper-floor ?to) (adj ?from ?to ?toDir) (is-west ?toDir) (facing-east ?p) (at ?p ?from) (zero-turn ?p) (not (jammed ?p)))
        :effect (and (not (at ?p ?from)) (at ?p ?to) (increase (total-cost) 2))
    )
    
    (:action move_back_from_east_one_bridge
        :parameters (?p - agent ?from ?to - square ?toDir - direction)
        :precondition (and (not (on-ground ?p)) (upper-floor ?from) (upper-floor ?to) (adj ?from ?to ?toDir) (is-west ?toDir) (facing-east ?p) (at ?p ?from) (first-turn ?p) (not (jammed ?p)))
    ...
    (:action move_back_from_west_zero_bridge
        :parameters (?p - agent ?from ?to - square ?toDir - direction)
        :precondition (and (not (on-ground ?p)) (upper-floor ?from) (upper-floor ?to) (adj ?from ?to ?toDir) (is-east ?toDir) (facing-west ?p) (at ?p ?from) (zero-turn ?p) (not (jammed ?p)))
        :effect (and (not (at ?p ?from)) (at ?p ?to) (increase (total-cost) 2))
    )
    
    (:action move_back_from_west_one_bridge
        :parameters (?p - agent ?from ?to - square ?toDir - direction)
        :precondition (and (not (on-ground ?p)) (upper-floor ?from) (upper-floor ?to) (adj ?from ?to ?toDir) (is-east ?toDir) (facing-west ?p) (at ?p ?from) (first-turn ?p) (not (jammed ?p)))
        :effect (and (not (at ?p ?from)) (at ?p ?to) (zero-turn ?p) (not (first-turn ?p)) (increase (total-cost) 2))
    )
    ...
    (:action move_back_from_north_zero_bridge
        :parameters (?p - agent ?from ?to - square ?toDir - direction)
        :precondition (and (not (on-ground ?p)) (upper-floor ?from) (upper-floor ?to) (adj ?from ?to ?toDir) (is-south ?toDir) (facing-north ?p) (at ?p ?from) (zero-turn ?p) (not (jammed ?p)))
        :effect (and (not (at ?p ?from)) (at ?p ?to) (increase (total-cost) 2))
    )
    
    (:action move_back_from_north_one_bridge
        :parameters (?p - agent ?from ?to - square ?toDir - direction)
        :precondition (and (not (on-ground ?p)) (upper-floor ?from) (upper-floor ?to) (adj ?from ?to ?toDir) (is-south ?toDir) (facing-north ?p) (at ?p ?from) (first-turn ?p) (not (jammed ?p)))
        :effect (and (not (at ?p ?from)) (at ?p ?to) (zero-turn ?p) (not (first-turn ?p)) (increase (total-cost) 2))
    )
    ...
    (:action move_back_from_south_zero_bridge
        :parameters (?p - agent ?from ?to - square ?toDir - direction)
        :precondition (and (not (on-ground ?p)) (upper-floor ?from) (upper-floor ?to) (adj ?from ?to ?toDir) (is-north ?toDir) (facing-south ?p) (at ?p ?from)  (zero-turn ?p) (not (jammed ?p)))
        :effect (and (not (at ?p ?from)) (at ?p ?to) (increase (total-cost) 2))
    )
    
    (:action move_back_from_south_one_bridge
        :parameters (?p - agent ?from ?to - square ?toDir - direction)
        :precondition (and (not (on-ground ?p)) (upper-floor ?from) (upper-floor ?to) (adj ?from ?to ?toDir) (is-north ?toDir) (facing-south ?p) (at ?p ?from) (first-turn ?p) (not (jammed ?p)))
        :effect (and (not (at ?p ?from)) (at ?p ?to) (zero-turn ?p) (not (first-turn ?p)) (increase (total-cost) 2))
    )
    ...
    (:action right_back_from_west_bridge
        :parameters (?p - agent ?from ?to - square ?toDir - direction)
        :precondition (and (not (on-ground ?p)) (upper-floor ?from) (upper-floor ?to) (adj ?from ?to ?toDir) (is-north ?toDir) (facing-west ?p) (at ?p ?from) (not (jammed ?p)))
    ...
    (:action left_back_from_west_bridge
        :parameters (?p - agent ?from ?to - square ?toDir - direction)
        :precondition (and (not (on-ground ?p)) (upper-floor ?from) (upper-floor ?to) (adj ?from ?to ?toDir) (is-south ?toDir) (facing-west ?p) (at ?p ?from) (not (jammed ?p)))
    ...
    (:action right_back_from_north_bridge
        :parameters (?p - agent ?from ?to - square ?toDir - direction)
        :precondition (and (not (on-ground ?p)) (upper-floor ?from) (upper-floor ?to) (adj ?from ?to ?toDir) (is-east ?toDir) (facing-north ?p) (at ?p ?from) (not (jammed ?p)))
    ...
    (:action left_back_from_north_bridge
        :parameters (?p - agent ?from ?to - square ?toDir - direction)
        :precondition (and (not (on-ground ?p)) (upper-floor ?from) (upper-floor ?to) (adj ?from ?to ?toDir) (is-west ?toDir) (facing-north ?p) (at ?p ?from)  (not (jammed ?p)))
    ...
    (:action right_back_from_south_bridge
        :parameters (?p - agent ?from ?to - square ?toDir - direction)
        :precondition (and (not (on-ground ?p)) (upper-floor ?from) (upper-floor ?to) (adj ?from ?to ?toDir) (is-west ?toDir) (facing-south ?p) (at ?p ?from)  (not (jammed ?p)))
    ...
    (:action left_back_from_south_bridge
        :parameters (?p - agent ?from ?to - square ?toDir - direction)
        :precondition (and (not (on-ground ?p)) (upper-floor ?from) (upper-floor ?to) (adj ?from ?to ?toDir) (is-east ?toDir) (facing-south ?p) (at ?p ?from)  (not (jammed ?p)))
    ...
    (:action right_back_from_east_bridge
        :parameters (?p - agent ?from ?to - square ?toDir - direction)
        :precondition (and (not (on-ground ?p)) (upper-floor ?from) (upper-floor ?to) (adj ?from ?to ?toDir) (is-south ?toDir) (facing-east ?p) (at ?p ?from) (not (jammed ?p)))
    ...
    (:action left_back_from_east_bridge
        :parameters (?p - agent ?from ?to - square ?toDir - direction)
        :precondition (and (not (on-ground ?p)) (upper-floor ?from) (upper-floor ?to) (adj ?from ?to ?toDir) (is-north ?toDir) (facing-east ?p) (at ?p ?from) (not (jammed ?p)))
    ...
    (:action throw_block
        ...
        :precondition (and (adj ?from ?to ?d) (block-at ?b1 ?from) (pit ?to) (agent-on-top ?p ?b1) (on ?b2 ?p) (not (bridge-base-built ?from ?to)))
        :effect (and (not (on ?b2 ?p)) (block-at ?b2 ?to) (is-free ?p) (bridge-base-built ?from ?to) (block-used-for-bridge ?b1) (block-used-for-bridge ?b2) (increase (total-cost) 1))
    )
    
    (:action build
        :parameters (?p - agent ?from ?to - square ?b1 ?b2 - block)
        :precondition (and (bridge-base-built ?from ?to) (agent-on-top ?p ?b1) (block-used-for-bridge ?b2))
        :effect (and (not (on ?b2 ?p)) (block-used-for-bridge ?b2) (upper-floor ?from) (upper-floor ?to) (increase (total-cost) 1))
    )
